...
populateLootList = function(parent, scrollFrame)
	...
	local factionMap = { Alliance = 1, Horde = 2 }
	local playerFactionId = factionMap[playerFaction] or 0

	for _, itemID in ipairs(items) do
		-- ʕ •ᴥ•ʔ✿ Faction filter: Only show if not in table, or if matches player faction ✿ ʕ •ᴥ•ʔ
		local lockedFaction = synastrialoot.hashFactionItems[itemID]
		if not lockedFaction or lockedFaction == playerFactionId then
